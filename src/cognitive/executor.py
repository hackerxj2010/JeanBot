...
class MissionExecutor:
    ...
    async def execute(self, record: MissionRecord, context: ExecutionContext) -> MissionRunResult:
        ...
        if existing_state:
            ...
            # Reconstruct StepExecutionRecords
            for report_data in existing_state.get("step_reports", []):
                diag_data = report_data.get("diagnostics")
                diagnostics = None
                if diag_data:
            ...
            # Reconstruct MissionArtifacts
            for art_data in existing_state.get("artifacts", []):
                artifacts.append(MissionArtifact(**art_data))

            # Update step statuses based on completed reports
