...
class MissionExecutor:
    ...
    async def execute(self, record: MissionRecord, context: ExecutionContext) -> MissionRunResult:
        ...
        if existing_state:
            ...
            for r in existing_state.get("step_reports", []):
                diag_data = r.get("diagnostics")
                diagnostics = None
                if diag_data:
                    diagnostics = StepExecutionDiagnostics(**diag_data)
            ...
            for a in existing_state.get("artifacts", []):
                artifacts.append(MissionArtifact(**a))

            # Sync plan status with step reports
            plan = self._require_plan(record)
            for report in step_reports:
                step = next((s for s in plan.steps if s.id == report.step_id), None)
                if step and report.diagnostics and report.diagnostics.failure_class == "none":
                    step.status = "completed"
        ...
        while remaining_steps:
            ...
            await self.file_service.save_mission_state(record.objective.id, {
                ...
                "step_reports": [
                    {
                        **r.__dict__,
                        "diagnostics": r.diagnostics.__dict__ if r.diagnostics else None
