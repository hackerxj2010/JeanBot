...
class MissionExecutorService:
    ...
    internal_token: str = field(
        default_factory=lambda: os.getenv("INTERNAL_SERVICE_TOKEN", "internal-secret-token")
    )
    ...
    def build_bundle(self, mission_payload: dict[str, Any]) -> MissionExecutionBundle:
        ...
        if self.service_mode == "http":
            audit_service = HttpAuditService(api_url=self.api_url, token=self.internal_token)
            memory_service = HttpMemoryService(api_url=self.api_url, token=self.internal_token)
            runtime_service = HttpRuntimeService(api_url=self.api_url, token=self.internal_token)
            subagent_service = HttpSubAgentService(api_url=self.api_url, token=self.internal_token)
            policy_service = HttpPolicyService(api_url=self.api_url, token=self.internal_token)
        else:
