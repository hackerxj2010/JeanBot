...
class MissionExecutorService:
    ...
    def build_bundle(self, mission_payload: dict[str, Any]) -> MissionExecutionBundle:
        ...
        mode = os.environ.get("JEANBOT_SERVICE_MODE", "local").lower()
        api_url = os.environ.get("JEANBOT_API_URL", "http://localhost:8080")
        internal_token = os.environ.get("INTERNAL_SERVICE_TOKEN", "default-token")
        ...
        if mode == "http":
            audit_service: AuditService = HttpAuditService(
                api_url=api_url,
                auth_token=internal_token,
                service_name="agent-orchestrator",
                auth_context=context.auth_context,
            )
            memory_service: MemoryService = HttpMemoryService(
                api_url=api_url,
                auth_token=internal_token,
                service_name="agent-orchestrator",
                auth_context=context.auth_context,
            )
            runtime_service: AgentRuntimeService = HttpRuntimeService(
                api_url=api_url,
                auth_token=internal_token,
                service_name="agent-orchestrator",
                auth_context=context.auth_context,
            )
            subagent_service: SubAgentService = HttpSubAgentService(
                api_url=api_url,
                auth_token=internal_token,
                service_name="agent-orchestrator",
                auth_context=context.auth_context,
