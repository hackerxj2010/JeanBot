...
class MissionExecutionBundle:
    ...
    subagent_service: MissionSubAgentService
...
class MissionExecutorService:
    ...
    service_mode: str = field(
        default_factory=lambda: os.getenv("JEANBOT_SERVICE_MODE", "local").lower()
    )
    api_url: str = field(
        default_factory=lambda: os.getenv("JEANBOT_API_URL", "http://localhost:8080")
    )
    internal_token: str = field(default_factory=lambda: os.getenv("INTERNAL_SERVICE_TOKEN", ""))
    ...
    def build_bundle(self, mission_payload: dict[str, Any]) -> MissionExecutionBundle:
        ...
        mode = mission_payload.get("mode", self.service_mode)
        api_url = mission_payload.get("api_url", self.api_url)
        internal_token = mission_payload.get("internal_token", self.internal_token)

        if mode == "http":
            audit_service: AuditService = HttpAuditService(api_url, internal_token)
            memory_service: MemoryService = HttpMemoryService(api_url, internal_token)
            subagent_service: MissionSubAgentService = HttpSubAgentService(api_url, internal_token)
        else:
    ...
    def _persist_run_summary(
        ...
        audit_summary = (
            bundle.audit_service.summarize()
            if hasattr(bundle.audit_service, "summarize")
            else {"mode": "unknown"}
        )
        memory_summary = (
            bundle.memory_service.summarize(bundle.record.objective.workspace_id)
