...
class MissionExecutorService:
    ...
    def plan_mission(self, mission_payload: dict[str, Any]) -> MissionPlan:
        plan = self._build_plan(mission_payload)
        if not plan.summary:
            plan.summary = f"Plan for objective: {mission_payload.get('objective', 'Unknown')}"
        return plan
    ...
    def get_mission_payload(self, mission_id: str) -> dict[str, Any] | None:
        mission_dir = self._mission_dir(mission_id)
        path = mission_dir / "mission-payload.json"
        if not path.exists():
            return None
        return json.loads(path.read_text(encoding="utf-8"))
    ...
    def list_missions(self) -> list[dict[str, Any]]:
        ...
        for mission_dir in missions_dir.iterdir():
            if not mission_dir.is_dir():
                continue
            summary = self.get_mission_run_summary(mission_dir.name)
            if summary:
                results.append({
                    "mission_id": mission_dir.name,
                    "title": summary.get("mission", {}).get("title"),
                    "status": summary.get("result", {}).get("status"),
                    "started_at": summary.get("result", {}).get("started_at"),
                })
