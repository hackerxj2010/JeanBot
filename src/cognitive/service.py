...
class MissionExecutorService:
    ...
    service_mode: str = field(
        default_factory=lambda: os.environ.get("JEANBOT_SERVICE_MODE", "local")
    )
    api_url: str = field(default_factory=lambda: os.environ.get("JEANBOT_API_URL", "http://localhost:8080"))
    internal_token: str = field(
        default_factory=lambda: os.environ.get("INTERNAL_SERVICE_TOKEN", "jeanbot-internal-dev-token")
    )
    ...
    def build_bundle(self, mission_payload: dict[str, Any]) -> MissionExecutionBundle:
        ...
        mode = mission_payload.get("service_mode", self.service_mode)
        api_url = mission_payload.get("api_url", self.api_url)
        token = mission_payload.get("internal_token", self.internal_token)

        if mode == "http":
            audit_service = HttpAuditService(api_url=api_url, token=token)
            memory_service = HttpMemoryService(api_url=api_url, token=token)
            ...
            # SubAgentService remains deterministic in the Python executor for now
            # as it relies on the other services for live ops.
    ...
    def _persist_run_summary(
        ...
        # Summaries might not exist for HTTP adapters in the same way
        audit_summary = bundle.audit_service.summarize() if hasattr(bundle.audit_service, "summarize") else {"mode": "http"}
        memory_summary = bundle.memory_service.summarize(bundle.record.objective.workspace_id) if hasattr(bundle.memory_service, "summarize") else {"mode": "http"}
