...
const contentHashFor = (value: string) =>
  // @ts-ignore
  crypto.hash ? crypto.hash("sha256", normalizeText(value), "hex") : crypto.createHash("sha256").update(normalizeText(value)).digest("hex");

const seededUnitValue = (seed: string, index: number) => {
  // @ts-ignore
  const digest = crypto.hash ? crypto.hash("sha256", `${seed}:${index}`, "buffer") : crypto.createHash("sha256").update(`${seed}:${index}`).digest();
