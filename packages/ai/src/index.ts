...
const syntheticVector = (text: string, dimensions = DEFAULT_EMBEDDING_DIMENSIONS) => {
  ...
  for (let index = 0; index < dimensions; index += 1) {
    ...
    // Use Math.round for faster numerical rounding compared to .toFixed(8)
...
const normalizeVector = (values: number[]) => {
  ...
  // Use manual for loop to avoid overhead of reduce/map in high-dimensional vectors
  let sum = 0;
  ...
  for (let index = 0; index < length; index += 1) {
    ...
    // Use Math.round for faster numerical rounding compared to .toFixed(8)
    result[index] = Math.round((val / magnitude) * 1e8) / 1e8;
  }
  return result;
};
