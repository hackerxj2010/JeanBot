...
const syntheticVector = (text: string, dimensions = DEFAULT_EMBEDDING_DIMENSIONS) => {
  ...
  for (let index = 0; index < dimensions; index++) {
    ...
    // High-performance alternative to toFixed(8) while maintaining parity
    values[index] = centered >= 0 ? Math.round(centered * 1e8) / 1e8 : -Math.round(-centered * 1e8) / 1e8;
  }
...
const normalizeVector = (values: number[]) => {
  ...
  for (let index = 0; index < values.length; index++) {
    const v = (values[index] ?? 0) * invMagnitude;
    // High-performance alternative to toFixed(8) while maintaining parity
    result[index] = v >= 0 ? Math.round(v * 1e8) / 1e8 : -Math.round(-v * 1e8) / 1e8;
  }
  return result;
};
