...
const contentHashFor = (value: string) => {
  ...
  // Ensure we always return a hex string for backward compatibility
...
const seededUnitValue = (seed: string, index: number) => {
  ...
  let digest: Buffer | Uint8Array;
  if (typeof (crypto as any).hash === "function") {
    digest = (crypto as any).hash("sha256", input);
  } else {
    digest = crypto.createHash("sha256").update(input).digest();
  }
  // Buffer and Uint8Array both have access to index 0, 1, 2, 3
  // But only Buffer has readUInt32BE. We can use DataView for a cross-platform way,
  // or just manual byte manipulation for performance.
...
const syntheticVector = (text: string, dimensions = DEFAULT_EMBEDDING_DIMENSIONS) => {
  ...
  for (let index = 0; index < dimensions; index++) {
    ...
    // Optimized: Math.round is much faster than .toFixed(8) which involves string conversion
  ...
  for (let i = 0; i < dimensions; i++) {
    values[i] = Math.round((values[i] * invMagnitude) * 1e8) / 1e8;
  }
  return values;
};
...
export const normalizeVector = (values: number[]) => {
  ...
  const result = new Array(length);
  for (let i = 0; i < length; i++) {
    // Optimized: Multiply by inverse magnitude instead of dividing, and use Math.round
...
export const cosineSimilarity = (left: number[] | undefined, right: number[] | undefined) => {
  ...
  for (let index = 0; index < length; index += 1) {
    const leftValue = (left[index] as number) ?? 0;
    const rightValue = (right[index] as number) ?? 0;
