...
const contentHashFor = (value: string) =>
  // @ts-ignore - Node 22 crypto.hash is ~74% faster for single-shot operations
  crypto.hash("sha256", normalizeText(value), "hex");

const seededUnitValue = (seed: string, index: number) => {
  // @ts-ignore - Node 22 crypto.hash is faster than createHash for small inputs
...
const syntheticVector = (text: string, dimensions = DEFAULT_EMBEDDING_DIMENSIONS) => {
  ...
  // Optimization: Manual loop and Math.round are significantly faster than Array.from and toFixed
  for (let i = 0; i < dimensions; i++) {
    const centered = seededUnitValue(hash, i) * 2 - 1;
    // High-performance toFixed(8) alternative with correct midpoint rounding for negative numbers
    values[i] =
      centered >= 0 ? Math.round(centered * 1e8) / 1e8 : -Math.round(-centered * 1e8) / 1e8;
  }
...
const normalizeVector = (values: number[]) => {
  ...
  // Optimization: Manual loop for sum of squares
  let sum = 0;
  ...
  // Optimization: Use reciprocal multiplication instead of division in loop
  const invMagnitude = 1 / magnitude;
  for (let i = 0; i < length; i++) {
    const v = values[i] * invMagnitude;
    // High-performance toFixed(8) alternative
